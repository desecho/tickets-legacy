...
common_fields_to_exclude = ('user_created', 'user_modified', 'date_created',
                            'date_modified')


class TicketForm(ModelForm):
    date_assigned = DateTimeField(
        label='Дата/Время', initial=date.today,
        widget=DateTimeInput(format=settings.FORMAT_DATE),
        input_formats=(settings.FORMAT_DATE,))
    time = TimeField(label='Длительность заявки',
                     initial=time(hour=settings.DEFAULT_TIME),
                     widget=TimeInput(format=settings.FORMAT_TIME),
                     input_formats=(settings.FORMAT_TIME,))
    ...
    def clean_date_assigned(self):
        ...
        def checkIfTimeIsFree(team, date_to_check_start, id, time):
            ...
            def checkIfTimespanOverlaps(date1_start, date1_end, date2_start,
                                        date2_end):
            ...
            def checkIfDayOff():
                ...
                return checkIfDayOfWeekInDayOfWeekList(date_to_check_start,
                                                       days_off)

            def checkIfDinnerTime():
                dinner_time = get_dinner_time()
                current_date = date(year=date_to_check_start.year,
                                    month=date_to_check_start.month,
                                    day=date_to_check_start.day)
                dinner_start = datetime.combine(current_date, dinner_time[0])
                dinner_end = datetime.combine(current_date, dinner_time[1])
                return checkIfTimespanOverlaps(date_to_check_start,
                                               date_to_check_end,
                                               dinner_start, dinner_end)
            tickets = Ticket.objects.filter(
                team=team, status=1,
                date_assigned__year=date_to_check_start.year,
                date_assigned__month=date_to_check_start.month,
                date_assigned__day=date_to_check_start.day)
            if id:
                tickets = tickets.exclude(pk=id)
            for ticket in tickets:
                date_to_compare = ticket.date_assigned
                if checkIfTimespanOverlaps(
                    date_to_check_start,
                    date_to_check_end,
                    date_to_compare,
                    date_to_compare + timedelta(minutes=ticket.time.minute,
                                                hours=ticket.time.hour)):
                    return (0, ticket.id)
        ...
        status, id = checkIfTimeIsFree(self.cleaned_data['team'],
                                       selected_date, self.id,
                                       getTime(self.cleaned_data['time']))
        if status != -1:
            if status == 0:
...
class AddTicketSaveForm(TicketForm):
    class Meta(TicketForm.Meta):
        exclude = ('date_created', 'date_modified', 'solution', 'status',
                   'reason')
